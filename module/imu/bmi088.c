	...
	if (_bmi088->cali_mode == BMI088_CALIBRATE_ONLINE_MODE) // 性感bmi088在线标定
	{
		...
		for (uint16_t i = 0 ; i < acc_cali_time ; i++)
		{
			...
			for (uint8_t j = 0 ; j < 3 ; j++)
			{
				if (j == 2)
				{
					// _bmi088->acc_offset[j] = bmi088_data.acc[j] * _bmi088->accel_scale - 9.81f; // 逐渐减小偏移量
					_bmi088->acc_offset[j] = (bmi088_data.acc[j] * _bmi088->accel_scale - 9.81f) * 0.25f + _bmi088->acc_offset[j] * 0.75f;
				}
				else
				{
