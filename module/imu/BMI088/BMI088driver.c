...
static uint8_t write_BMI088_gyro_reg_data_error[BMI088_WRITE_GYRO_REG_NUM][3] =
		{
				...
				{
				  BMI088_GYRO_BANDWIDTH,         BMI088_GYRO_2000_230_HZ |
...
static uint8_t BMI088_Base_Init( SPI_HandleTypeDef *bmi088_SPI, uint8_t calibrate )
{
...
{
	while( BMI088_Base_Init( bmi088_SPI, calibrate ) )
	{ ;
	}
}
...
void Calibrate_MPU_Offset( IMU_data_t *bmi088 )
{
	...
	for( uint16_t i = 0; i < acc_CaliTimes; i++ )
	{
		...
		bmi088->AccelOffset[0] = bmi088->AccelOffset[0] * 0.75f + bmi088->Accel[0] * 0.25f;//加速度一阶低通滤波

		bmi088_raw_temp = ( int16_t )( ( buf[3] ) << 8 ) | buf[2];
		bmi088->Accel[1] = bmi088_raw_temp * BMI088_ACCEL_SEN * bmi088->AccelScale;
		bmi088->AccelOffset[1] = bmi088->AccelOffset[1] * 0.75f + bmi088->Accel[1] * 0.25f;//加速度一阶低通滤波

		bmi088_raw_temp = ( int16_t )( ( buf[5] ) << 8 ) | buf[4];
		bmi088->Accel[2] = bmi088_raw_temp * BMI088_ACCEL_SEN * bmi088->AccelScale;
		bmi088->AccelOffset[2] = bmi088->AccelOffset[2] * 0.75f + bmi088->Accel[2] * 0.25f;//加速度一阶低通滤波
...
void BMI088_Read( IMU_data_t *bmi088 )
{
	...
	//读取三轴角速度
	BMI088_gyro_read_muli_reg( BMI088_GYRO_CHIP_ID, buf, 6 );
	if( buf[0] == BMI088_GYRO_CHIP_ID_VALUE )
	{
		if( caliOffset )
		{
