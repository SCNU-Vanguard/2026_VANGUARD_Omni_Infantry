...
typedef struct
{
  ...
  uint8_t shoot_mode;				 // 是否开火
  uint8_t friction_mode;			 // 摩擦轮是否开启
  uint8_t auto_mode;					 // 自瞄是否打开
  float Chassis_Power_Measure; // 超电功率测量值
  uint8_t SuperCap_Energy;
  uint8_t Tunnel_State;     //过隧道时状态（处于 下 中 上） 1 2 3
  ...
  uint8_t auto_last_mode;
  float Chassis_last_Power_Measure;
  uint8_t SuperCap_last_Energy;
  uint8_t Tunnel_last_State;
